...
def analisar_ano(ano: int, questoes: List[Dict], final_data: List[Dict], skip_data: List[Dict], image_data: List[Dict]) -> None:
    print(f"\n=== {ano} ===")
    ...
    image_ids = [item['id'] for item in image_data if item.get('edicao') == ano]
    ...
    print(f" - Em image.json: {len(image_ids)}")
...
def main():
    ...
    image_data = carregar_dados('image.json', 'image')

    todos_dados = final_data + skip_data + image_data

    print("\n=== RESUMO ===")
    print(f"Total de questões validadas: {len(final_data)}")
    print(f"Total de questões skipadas (Tabelas): {len(skip_data)}")
    print(f"Total de questões com imagens: {len(image_data)}")
    ...
    for ano in range(2002, 2013):
        if ano != 2020 and ano != 2021:
            ...
            analisar_ano(ano, questoes_ano, final_data, skip_data, image_data)
